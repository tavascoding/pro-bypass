...
auto name = skCrypt("BYPASS PRO");
auto ownerid = skCrypt("57lpCg63QN");
auto secret = skCrypt("30e329b66baa7deda8b8c4a0cc5d7eab5739d74b7245bd2712ae658caf01cf14");
auto version = skCrypt("1.3");
...
{
    State = skCrypt("Start Bypass..").decrypt();
    ...
    while (AutoInject) // Kontrolowana pętla
    {
        ...
        if (procP2ID == 0) {
            ...
            std::cout << "Status: " << State << std::endl;
            Sleep(1500);
            State = skCrypt("core.dll randomize").decrypt();
            std::cout << "Status: " << State << std::endl;
            AutoInject = false;
        }
    }
    ...
    State = skCrypt("run star.bat now! (safety).").decrypt();
    std::cout << "Status: " << State << std::endl;
    if (hThread != NULL) {
        TerminateThread(hThread, 0);
        CloseHandle(hThread);
        hThread = NULL;
        std::cout << "After inject close manual console" << std::endl;
    }
}
            ...
            ImGui::PushStyleVar(ImGuiStyleVar_ChildRounding, 3.f);
            {
                ...
                if (show_login) {
                    ...
                    ImGui::BeginChild(skCrypt("##Authentication").decrypt(), ImVec2(300, 276), true);
                    {
                        ...
                        ImGui::TextDisabled(skCrypt("PRO BYPASS NEROCODE").decrypt());
                ...
                if (show_main_menu) {
                    ...
                    ImGui::SetCursorPos(ImVec2(118, 20));
                    ImGui::TextDisabled(skCrypt("PRO BYPASS (NERO)").decrypt());
                    ...
                    ImGui::BeginChild(skCrypt("##Core").decrypt(), ImVec2(300, 350), true);
                    {
                        ...
                        ImGui::TextDisabled(skCrypt("CORE SETTINGS").decrypt());

                        ImGui::Text(skCrypt("SELECT SERVER").decrypt());
                        ...
                        ImGui::Checkbox(skCrypt("MASK START.BAT FILE (SAFETY)").decrypt(), &AutoInject);
